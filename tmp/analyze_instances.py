...
for target_type, label in [(11, 'DUNGEON'), (6, 'ADVENTURE'), (9, 'RAID')]:
    print(label + ':')
    for i in range(wh['rec_count']):
        ...
        name = getstr(o1, o2)
        entries = sorted(wl2_by_world.get(wid, []))
        print(f'  World {wid:5d} [{name}]  ({len(entries)} WL2)')
        for e in entries[:6]:
            wl2id, x, y, z = e
            print(f'    WL2 {wl2id:6d}: ({x:9.2f},{y:9.2f},{z:9.2f})  dist={dist(x,y,z):.1f}')
    print()
